...
def make_image_path(volume_name, image_name):
    if not image_name:
        return ''
    return app.config['GITHUB_IMAGE_PATH_PREFIX'].format(volume_name=volume_name,
...
def make_image_url(image_path):
    ...
    """
    if image_path:
        return app.config['GITHUB_IMAGE_URL'].format(path=image_path)
    else:
        return image_path
...
def generate_out_url(image_path):
    ...
    """
    if image_path:
        return app.config['GITHUB_IMAGE_PREFIX']+image_path
    else:
        return image_path
